	...
	uint32_t inputWidth, uint32_t inputHeight, float boxHalfSize) const
{
	...
	uint32_t inputLevel(GetTextureLevel(inputWidth, baseSATA->GetWidth()));
...
inline uint32_t AdaptiveLayerGenerator::GetPowerOfTwoHigherThan(uint32_t val) const
{
	--val;
	val |= val >> 1;
	val |= val >> 2;
...
inline uint32_t AdaptiveLayerGenerator::GetTextureLevel(uint32_t inputWidth, uint32_t outputWidth) const
{
	uint32_t level = 0;
